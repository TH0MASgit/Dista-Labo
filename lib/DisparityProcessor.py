...
class DisparityProcessor:
    ...
    def __init__(self, settings={}):
        ...
        ## Frame holder for cuda: this is incorrect TODO : remove soon
        if args.bmcuda:
            self.gpu_frame_left = cv2.cuda_GpuMat()
            self.gpu_frame_right = cv2.cuda_GpuMat()
        ...
        # Disparity Params
        self.disparityDownScale = args.dispdown
        ...
        blockSize = 5
        if args.bmcuda:
            self.left_matcher = cv2.cuda_StereoBM(numDisparities=self.num_disp, blockSize=blockSize)
        elif args.bm :
...
Compute the disparity Map from a left & right picture took from a camera
        ...
        if  args.bm:
            disparityLeft = self.left_matcher.compute(grayedImageLeft, grayedImageRight)
            disparityRight = self.right_matcher.compute(grayedImageRight, grayedImageLeft)
        elif args.bmcuda:
            self.gpu_frame_left=cv2.cuda_GpuMat()
            self.gpu_frame_right=cv2.cuda_GpuMat()
            self.gpu_frame_left.upload(grayedImageLeft)
            self.gpu_frame_right.upload(grayedImageRight)
            gpudisparityLeft = self.left_matcher.compute(self.gpu_frame_left, self.gpu_frame_right)
            gpudisparityRight = self.right_matcher.compute(self.gpu_frame_right,self.gpu_frame_left)
            disparityLeft=gpudisparityLeft.download()
            disparityRight=gpudisparityRight.download()
        else:
