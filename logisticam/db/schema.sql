/* Use this script to prepare an empty database for use with Dista.
   This file must be run through the C preprocessor (cpp -E) to replace defines.
   ...
   3.1 Minor updates to heatmap view and frames
...
#ifndef SQLITE
/* Don't forget to set the timezone after connecting to the database:
SET time_zone = 'America/Toronto';
*/
    #define LOCAL_TIME(expr) expr
    #define IIF(cond, val_true, val_false) IF(cond, val_true, val_false)
#else
    #define LOCAL_TIME(expr) datetime(expr, 'unixepoch', 'localtime')
    #define CURRENT_TIMESTAMP (strftime('%s', 'now'))
    #define AUTO_INCREMENT AUTOINCREMENT
    #define UNSIGNED

    #define META(_1, _2, _3, name, ...) name    
    #define CAT1(val1) val1
    #define CAT2(val1, val2) val1 || val2
    #define CATN(val1, ...) val1 || META(__VA_ARGS__, CONCAT, CAT2, CAT1)(__VA_ARGS__)
    #define CONCAT(...) META(__VA_ARGS__, CATN, CAT2, CAT1)(__VA_ARGS__)
    
    #define GREATEST(val1, ...) max(val1, __VA_ARGS__)
#endif
...
CREATE TABLE Cameras ( -- TODO many of the properties below should be links to constants
    ...
    port_no     SMALLINT(5) UNSIGNED,   -- port number or device id assigned to this camera
...
SELECT
    ...
    CONCAT(Machines.address, ':', Cameras.port_no) AS ip_address,
...
CREATE TABLE Sessions (
    id              INTEGER PRIMARY KEY AUTO_INCREMENT, -- PK: session identifier
...
CREATE TABLE Frames (
    id          INTEGER PRIMARY KEY AUTO_INCREMENT, -- PK: frame identifier
    ...
    duration    SMALLINT(3) UNSIGNED DEFAULT 1, -- Duration of the frame in ms
...
CREATE TABLE Detections (
    ...
    min_distance INTEGER UNSIGNED NULL, -- cached field: distance to nearest neighbor
...
CREATE TABLE Distances (
    ...
    distance    INTEGER UNSIGNED NOT NULL, -- distance between a pair of detections (in cm)
...
SELECT
    LOCAL_TIME(Frames.created_at) AS local_time,
...
#else
/* MySQL supports stored procedures, so there's a better tiled heatmap in procedures.sql */
#endif
CREATE VIEW Heatmap AS
SELECT
    local_time,
    camera_desc,
    CONCAT(frame_id, '-', detection_id) AS detection_id,
    pos_x/100.0 AS pos_X, pos_y/100.0 AS pos_Y, -- Display positions in m
    GREATEST(1.0 - IFNULL(min_distance / 200.0, 1.0), 0.0) AS collision_intensity
...
SELECT
    IIF(distance_count > 0,
        CONCAT(distance_count, ' redundant or invalid distances found'),
        'OK'
    ) AS distance_result,
    IIF(min_distance_count > 0,
        CONCAT(min_distance_count, ' cached min. distances are inconsistent'),
        'OK'
